...
const Portfolio = ({ gameState, marketState }: PortfolioProps) => {
  ...
  const totalValue = portfolioValueHistory.length > 0
  ...
  const dailyChangePercent = previousDayValue > 0 
    ? (dailyChange / previousDayValue) * 100 
    : 0;
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold mt-2">AED {cash.toFixed(2)}</p>
        </Card>
        ...
        <Card className="p-4">
          ...
          <div className="flex items-baseline space-x-2 mt-2">
            <p className={`text-2xl font-bold ${dailyChange >= 0 ? 'text-profit' : 'text-loss'}`}>
              {dailyChange >= 0 ? '+' : ''}{dailyChange.toFixed(2)}
            </p>
            <p className={`${dailyChange >= 0 ? 'text-profit' : 'text-loss'}`}>
        ...
        <Card className="p-4">
          ...
          <div className="flex items-baseline space-x-2 mt-2">
            <p className={`text-2xl font-bold ${totalChange >= 0 ? 'text-profit' : 'text-loss'}`}>
              {totalChange >= 0 ? '+' : ''}{totalChange.toFixed(2)}
            </p>
            <p className={`${totalChange >= 0 ? 'text-profit' : 'text-loss'}`}>
      ...
      <PortfolioChart portfolioHistory={portfolioValueHistory} />
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {holdings.map((holding) => {
                  ...
                  const currentPrice = getStockPrice(holding.companyId, currentDay, priceData) || 0;
                  const currentValue = currentPrice * holding.shares;
                  
                  // Calculate average purchase price
                  const totalPurchaseCost = holding.purchaseHistory.reduce(
                    (acc, purchase) => acc + purchase.price * purchase.shares,
                    0
                  );
                  const totalShares = holding.shares;
                  const avgPurchasePrice = totalShares > 0 ? totalPurchaseCost / totalShares : 0;
                  
                  // Calculate gain/loss
                  const gainLoss = currentValue - totalPurchaseCost;
                  const gainLossPercent = totalPurchaseCost > 0 
                    ? (gainLoss / totalPurchaseCost) * 100 
                    : 0;
                  ...
                  return (
                    <tr key={holding.companyId}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-gray-500 text-sm">{company?.ticker || holding.companyId}</div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        ...
                        <div className="text-gray-500 text-sm">
                          avg: {avgPurchasePrice.toFixed(2)}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className={`font-medium ${gainLoss >= 0 ? 'text-profit' : 'text-loss'}`}>
                          {gainLoss.toFixed(2)}
                        </div>
                        <div className={`text-sm ${gainLoss >= 0 ? 'text-profit' : 'text-loss'}`}>
