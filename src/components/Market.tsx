...
const Market = ({ 
  ...
  const renderPriceChange = (companyId: string) => {
    ...
    const yesterday = currentDay > 1 ? getStockPrice(companyId, currentDay - 1, priceData) : null;
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="space-y-4 flex-grow">
                <div className="space-y-2">
                  <label className="block text-sm">Select Company</label>
                ...
                {info && (
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-lg font-bold">AED {info.price.toFixed(2)}</p>
                      {renderPriceChange(selectedCompany)}
                    </div>
                    
                    <div>
                      <p className="text-sm">You Own:</p>
                      <p>{info.sharesOwned.toFixed(2)} shares</p>
                    </div>
                    
                    <div className="space-y-2">
                      <label className="block text-sm">Amount to Trade</label>
                      <Input
                        type="number"
                        min="0.01"
                        step="0.01"
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold">AED {info.totalCost.toFixed(2)}</p>
                    </div>
                    
                    <div className="flex gap-2 mt-4">
                      <Button 
                        className="flex-1"
                        onClick={() => onBuy(selectedCompany, shares)}
                      ...
                      <Button 
                        className="flex-1" 
                        variant="outline"
                        onClick={() => onSell(selectedCompany, shares)}
                        disabled={shares > info.sharesOwned}
                      >
                        Sell
                      </Button>
                    </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {companies.map((company) => {
                ...
                return (
                  <tr 
                    key={company.id}
                    className={`hover:bg-gray-50 cursor-pointer ${selectedCompany === company.id ? 'bg-gray-100' : ''}`}
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-gray-500 text-sm">{company.ticker}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-gray-900">
                      {sharesOwned > 0 ? sharesOwned.toFixed(2) : '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-gray-900">
                      {sharesOwned > 0 ? holdingValue.toFixed(2) : '-'}
                    </td>
                  </tr>
                );
              })}
