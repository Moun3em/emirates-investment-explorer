...
const Index = () => {
	...
	useEffect(() => {
		if (
			appState.gameState?.isGameOver &&
			appState.gameState.currentDay > 5
		) {
	...
	const handleStartGame = (startingCapital: number, tradesPerDay: number) => {
		...
		toast.success(
			"New game started with AED " + startingCapital.toFixed(2)
		);
	};
	...
	const handleAdvanceDay = () => {
		...
		const updatedGameState = advanceToNextDay(
			appState.gameState,
			appState.marketState
		);
	...
	const handleBuyStock = (companyId: string, shares: number) => {
		...
		const updatedGameState = buyStock(
			appState.gameState,
			appState.marketState,
			companyId,
			shares
	...
	const handleSellStock = (companyId: string, shares: number) => {
		...
		const updatedGameState = sellStock(
			appState.gameState,
			appState.marketState,
			companyId,
			shares
	...
	// Handle stopping the game early
	...
	// Handle continuing the game after stopping
			...
			<div className="min-h-screen flex items-center justify-center bg-gray-50">
				<div className="text-center space-y-4">
					...
					<p className="text-gray-600">
						Learn about investing in the UAE stock market!
					</p>
